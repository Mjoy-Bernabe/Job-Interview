...
def _clean(raw: str) -> str:
    ...
    Also converts PDF CID glyph references to their actual characters:
      ...
      (cid:151) → em-dash  —
    """
    # CID → unicode replacements (cover all common glyph encodings)
    cid_map = {
        "(cid:127)": "• ",   # bullet
...
# Curated list of skills/roles that actually appear in resumes
KNOWN_SKILLS = [
    # Languages
...
# ═══════════════════════════════════════════════════════════════════════════════
# Job-description keyword extractor  (replaces the noisy bigram version)
# ═══════════════════════════════════════════════════════════════════════════════

# Skills/roles that are actually checkable in a resume
_CHECKABLE_TERMS = [s.lower() for s in KNOWN_SKILLS] + [
...
def extract_keywords_from_job_desc(description: str, job_name: str = "") -> list[str]:
    """
    Pull known-skill / role-name terms from the job description text.
    If the description yields fewer than 5 keywords, supplements with
    role-based implied skills looked up from the job name.
    Returns a deduplicated list (original casing from KNOWN_SKILLS).
    """
...
def extract_skills_from_text(text: str, extra_keywords: list = None) -> list:
    found = set()
    lower = text.lower()
...
def extract_all_info(text: str, job_keywords: list = None) -> dict:
    """
