...
CREATE TABLE job_desc (
    ...
    required_degree VARCHAR(100) NULL,
    required_course VARCHAR(100) NULL,
...
-- ==========================================
-- Migration: add applicant personal fields + job requirement fields
-- Safe to run against an existing auth_db without dropping data.
-- ==========================================

ALTER TABLE applicants
    ADD COLUMN civil_status ENUM('Single','Married','Widowed','Separated','Divorced') NULL AFTER date_of_birth,
    ADD COLUMN gender ENUM('Male','Female','Other','Prefer not to say') NULL AFTER civil_status,
    ADD COLUMN nationality VARCHAR(50) NULL AFTER preferred_location,
    ADD COLUMN phone_number VARCHAR(20) NULL AFTER nationality,
    ADD COLUMN linkedin_url VARCHAR(255) NULL AFTER phone_number,
    ADD COLUMN portfolio_url VARCHAR(255) NULL AFTER linkedin_url;

ALTER TABLE job_desc
    ADD COLUMN work_setup ENUM('Onsite','Hybrid','Remote') NOT NULL DEFAULT 'Onsite' AFTER employment_type,
    ADD COLUMN work_schedule VARCHAR(100) NULL AFTER work_setup,
    ADD COLUMN required_degree VARCHAR(100) NULL AFTER education_baseline,
