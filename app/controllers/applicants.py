...
from app.services.email_service import send_step1_completed_email  # ⬅️ NEW IMPORT
...
from app.services import resume_scanner
...
#
# NOTE: This used to instantiate its OWN DecisionTreeClassifier, trained at
# import time on `raw_applicants_cart` — 10 hand-typed rows. That model was
# completely disconnected from ats_resume_dataset_elite_v3.csv and from the
# dataset-trained model the resume-upload flow (/submit-resume) already
# uses (services/resume_scanner.get_cart_model(), see cart_model.py). Two
# applicants with identical qualifications could get different verdicts
# purely depending on whether they applied via the manual form or by
# uploading a resume.
#
# There is now exactly ONE trained model in the project — the
# DecisionTreeClassifier cached in dataset_fit_model.pkl / trained from
# ats_resume_dataset_elite_v3.csv (see cart_model.py). The manual forms
# below (`/submit`, `/prescreenn`) don't have a resume to parse, so they
# call `resume_scanner.predict_fit_from_match_scores()` — a dataset-only
# entry point that skips the resume-structure rule score (bullets,
# sections, etc.) and uses just the three columns the dataset model was
# actually trained on: skill_match_score, experience_match, education_match.

# Maps the manual form's `education_level` select values onto the same
# degree-level vocabulary resume_scanner.evaluate_education_requirement()
# uses for uploaded resumes (see DEGREE_LEVEL_ORDER / DEGREE_KEYWORDS in
# resume_parser.py), so a typed-in education level and a parsed resume
# education level are judged against a job's requirement the same way.
CART_EDU_TO_DEGREE_LEVEL = {
    "high_school": None,  # below the lowest tracked level (Entry-level track)
    "vocational": "Entry-level track",
    "associate": "Associate",
    "bachelor": "Bachelor's",
    "master": "Master's",
    "phd": "Doctoral",
...
def cart_skill_score(skill_count: int) -> float:
    """
    Approximates skill_match_score for the manual form, where — unlike the
    resume-upload flow — we don't have the job's parsed keyword list to
    check the applicant's skills against directly. Caps at 10 listed
    skills, matching the scale the dataset model was trained on.
    """
    return min(skill_count, 10) / 10.0


def cart_predict_from_form(
    age,
    education_level,
    experience,
    skills_raw: str,
    required_exp_years: float = 0,
    education_baseline: str | None = None,
):
    """
    Build the three features the real dataset model expects
    (skill_match_score, experience_match, education_match) from manual
    form input, then predict through the shared dataset-trained model.

    `required_exp_years` / `education_baseline` should come from the job
    posting (job_desc table) when available, so experience_match /
    education_match reflect this applicant vs. THIS job — not a guess.
    When no job context exists (e.g. the legacy self-check in
    /prescreenn), education_match defaults to True and experience_match
    to `experience > 0`, matching resume_scanner's own convention of
    treating an undetected requirement as satisfied.
    """
    skills_list = [s.strip() for s in skills_raw.split(",") if s.strip()]
    skill_count = len(skills_list)
    skill_match_score = cart_skill_score(skill_count)

    experience_match = experience >= (required_exp_years or 0)

    edu_clean = (education_level or "").lower().replace("'", "").replace(" ", "_")
    mapped_degree_level = CART_EDU_TO_DEGREE_LEVEL.get(edu_clean)
    education_check = resume_scanner.evaluate_education_requirement(
        [{"degree_level": mapped_degree_level}] if mapped_degree_level else [],
        education_baseline,
    )
    education_match = education_check["meets"]

    result = resume_scanner.predict_fit_from_match_scores(
        skill_match_score=skill_match_score,
        experience_match=experience_match,
        education_match=education_match,
    )

    # Keep the extra context around (age isn't used by the model — the
    # dataset it's trained on doesn't include age — but templates/session
    # may still expect these keys, so they're preserved for compatibility).
    result["features"]["age"] = age
    result["features"]["skill_count"] = skill_count
    result["features"]["required_exp_years"] = required_exp_years
    result["features"]["education_baseline"] = education_baseline
    return result
...
# ---------- Job-required-skills lookup (real HR-curated source of truth) ----------
#
# job_required_skills (job_desc_id, skill_id) joined with skills_master
# (skill_id, skill_name) is the actual list of skills HR configured for a
# given job posting (see phpMyAdmin: job_required_skills rows for job_desc_id
# 11 = Project Analyst, etc). Previously the resume-upload flow ignored this
# table entirely and called resume_scanner.extract_keywords_from_job_desc()
# — a regex/role-name GUESSER that resume_parser.py itself documents as
# "DEPRECATED / LAST-RESORT FALLBACK ONLY" — meaning every job effectively
# got the same generic guessed skill list instead of its own configured one.
# These two helpers pull the REAL per-job data instead.

def _job_required_skills(cur, job_id):
    """
    Returns [(skill_id, skill_name), ...] for this job's real,
    HR-curated required skills — resolved via job_desc_id, not job_id.
    """
    cur.execute(
        """
        SELECT s.skill_id, s.skill_name
        FROM job_desc jd
        JOIN job_required_skills jrs ON jrs.job_desc_id = jd.job_desc_id
        JOIN skills_master s ON s.skill_id = jrs.skill_id
        WHERE jd.job_id = %s
        """,
        (job_id,),
    )
    return cur.fetchall()  # [(skill_id, skill_name), ...]


def _all_known_skills(cur) -> list[str]:
    """
    Full skills_master vocabulary (every skill HR has ever defined across
    all jobs), so the parser can recognize any skill mentioned in a resume
    — not only the ones required by this particular job posting.
    """
    cur.execute("SELECT skill_name FROM skills_master")
...
def submit_application():
    ...
    try:
        ...
        # NOTE: now also pulls jd.education_baseline, so the manual-form
        # CART prediction below can judge education_match against this
        # job's real requirement (same helper the resume-upload flow uses)
        # instead of a hardcoded assumption.
        cur.execute(
            """
            SELECT j.job_id, j.max_applicants, j.application_status, 
                   jd.minimum_age, jd.required_exp_years, jd.education_baseline
            FROM jobs j
            LEFT JOIN job_desc jd ON jd.job_id = j.job_id
            WHERE j.job_name = %s
            """,
        ...
        job_id, max_allowed, app_status, req_age, req_exp, education_baseline = job_info
        ...
        try:
            cart_result = cart_predict_from_form(
                ...
                skills_raw=skills,
                required_exp_years=req_exp or 0,
...
def prescreen():
    ...
    # No job_id is attached to this form, so there's no job requirement to
    # compare against — required_exp_years/education_baseline are left at
    # their defaults (0 / None), which cart_predict_from_form treats the
    # same way resume_scanner treats an undetected job requirement: as
    # satisfied. This is a general self-assessment, not a per-job screen.
...
# ─────────────────────────────────────────────────────────────────────────
# This is the corrected submit_resume_application() function from
# routes/applicants.py. Replace the existing function (from its
# @applicants_bp.route("/submit-resume", ...) decorator down through its
# closing `finally: cur.close()`) with this version. Nothing else in
# routes/applicants.py needs to change.
# ─────────────────────────────────────────────────────────────────────────
...
def submit_resume_application():
    ...
    contact info, skills, education, work experience) -> compute_weighted_fit
    ("Strong Fit" / "Moderate Fit" / "Weak Fit"), which blends the trained
    CART model's probability in as one of five weighted signals (education,
    experience, skills, age, CART — see cart_model.compute_weighted_fit).
    Nothing is written to the database at this point. The scanned/parsed
    result is parked on disk as a "pending scan" (see _save_pending_scan)
    and the applicant is shown resume_review.html — a Resume Screening
    Result card where they can double-check (and manually correct) whatever
    the scanner picked up before anything is saved. Confirming there posts
    to /confirm-resume-application (see below), which is the step that
    ...
    try:
        ...
        # ── 5. Load the REAL per-job required skills + full skill vocabulary ──
        # job_required_skills is HR-curated (see job_required_skills /
        # skills_master tables) — this is the actual source of truth for
        # what THIS job needs, not a regex guess off the job description.
        required_skills = _job_required_skills(cur, job_id)   # [(skill_id, name), ...]
        if not required_skills:
            guessed_names = resume_scanner.extract_keywords_from_job_desc(
                description=description or "", job_name=job_name or ""
            )
            required_skills = [(None, name) for name in guessed_names]

        job_keywords = [name for _, name in required_skills]   # strings only, for text matching

        # ── 6. Extract everything from the resume ONCE ───────────────────────
        # (Previously this ran twice — once to feed a predict_fit() call
        # whose result was thrown away, and again to feed compute_weighted_fit().
        # There is now a single extraction, reused for every downstream step.)
        known_skills = _all_known_skills(cur)
        extracted = resume_scanner.extract_all_info(
            text, job_keywords=job_keywords, known_skills=known_skills
        )
        ...
        # ── 7. Trained-model signal (dataset-calibrated CART classifier) ─────
        # Blended into compute_weighted_fit() below as one of five weighted
        # signals (see cart_model.WEIGHT_CART) — not the sole decision, and
        # not silently discarded like the old predict_fit() call was.
        resume_scanner.get_cart_model()  # warms/loads the cached model
        cart_probability = resume_scanner.dataset_fit_probability(
            features,
        ...
        required_skills_found = [
            ...
            for sid, name in required_skills
            if features["keyword_hits"].get(name, 0) > 0
        ]
        ...
        required_skills_total_count = len(required_skills)
        required_skills_found_count = len(required_skills_found)

        applicant_age = None
        dob_raw = contact_info.get("date_of_birth")
        if dob_raw:
            try:
                dob_parsed = datetime.strptime(dob_raw, "%Y-%m-%d")
                today = datetime.now()
                applicant_age = today.year - dob_parsed.year - (
                    (today.month, today.day) < (dob_parsed.month, dob_parsed.day)
                )
            except ValueError:
                applicant_age = None

        fit_result = resume_scanner.compute_weighted_fit(
            ats_compliance=extracted["ats_compliance"],
            highest_education_level=education_check.get("highest_level"),
            required_education_level=education_check.get("required_level"),
            applicant_experience_years=applicant_years,
            required_experience_years=required_exp_years or 0,
            required_skills_found_count=required_skills_found_count,
            required_skills_total_count=required_skills_total_count,
            applicant_age=applicant_age,
            minimum_age=minimum_age,
            cart_probability=cart_probability,
        )
        is_eligible = fit_result["eligible"]
        ats_check = fit_result["ats_check"]   # {"passed","ratio","score","max_score","failed_checks"}

        other_skills = [s for s in extracted["skills"] if s not in features["keyword_hits"]]

        # ── 8. Park everything on disk as a "pending scan" — NOT saved yet ──
        pending = {
            "user_id": user_id,
            "job_id": job_id,
            "job_name": job_name,
            ...
            "match_score": round(fit_result["score_percent"]),
            ...
            "minimum_age": minimum_age,
            "age_meets": (
                fit_result.get("age_score") is not None and fit_result["age_score"] >= 1.0
                if fit_result.get("stage_failed") is None else False
            ),
            "cart_score": fit_result.get("cart_score"),
            "stage_failed": fit_result["stage_failed"],
            "ats_checks": extracted["ats_compliance"]["checks"],
            "ats_score": ats_check["score"],
            "ats_max_score": ats_check["max_score"],
            "ats_ratio": ats_check["ratio"],
            "ats_passed": ats_check["passed"],
        }
        ...
        logger.error(f"submit_resume_application error: {e}", exc_info=True)
        if save_path and os.path.exists(save_path):
            try:
                os.remove(save_path)
            except OSError:
                pass
        # Show the real error message in dev so this isn't a black box;
        # RESUME_SCAN_DEBUG defaults to True — set it to False in
        # production config once this is confirmed working end-to-end.
        if current_app.config.get("RESUME_SCAN_DEBUG", True):
            flash(f"Something went wrong while scanning your resume: {e}", "error")
        else:
...
def confirm_resume_application():
    ...
    try:
        ...
        mysql.connection.rollback()
        logger.error(f"confirm_resume_application error: {e}", exc_info=True)
